...
**Index FULLTEXT — non câblé (code mort).** `npc_database.cpp` et `item_database.cpp` contiennent des helpers `hasNpcsFulltextIndex`/`hasItemsFulltextIndex` qui *feraient* `ALTER TABLE … ADD FULLTEXT INDEX ft_name`, et la migration `docs/sql_migrations/add_fulltext_indexes.sql` existe — **mais rien n'est actif** : ces helpers ne sont appelés nulle part et aucune requête n'utilise `MATCH … AGAINST`. Les recherches NPC (`searchNpcs`) et items tournent toujours en `WHERE … LIKE '%term%'` (full-scan). L'index `ft_name` n'est donc ni créé automatiquement, ni exploité. À câbler (appeler le helper au démarrage + passer les recherches en FULLTEXT) ou à retirer.
